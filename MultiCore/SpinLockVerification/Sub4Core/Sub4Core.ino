/*
 *  Sub4Core.ino - SubCore 4 sketch for SpinLockVerification multi-core sample.
...
void loop()
{
  ...
  if (locked) {
    ...
    g_report->owner = OWNER_SUB4;
    g_report->entries_sub4++;
    delay(HOLD_MS);
    g_report->owner = 0u;
    MultiCoreSpin::release(g_lock);
  } else {
    g_report->timeouts_sub4++;
  }
