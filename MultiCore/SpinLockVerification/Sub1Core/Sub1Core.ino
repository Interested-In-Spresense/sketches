/*
 *  Sub1Core.ino - SubCore 1 sketch for SpinLockVerification multi-core sample.
...
void loop()
{
  ...
  if (locked) {
    ...
    g_report->owner = OWNER_SUB1;
    g_report->entries_sub1++;
    delay(HOLD_MS);
    g_report->owner = 0u;
    MultiCoreSpin::release(g_lock);
  } else {
    g_report->timeouts_sub1++;
  }
